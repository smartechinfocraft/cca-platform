...
const MAX_IMG_SIZE     = 5 * 1024 * 1024;   // 5 MB per image
...
function ImageUploadField({ label, currentUrl, onFileSelect, required, hint }) {
  ...
  const validate = (file) => {
    ...
    if (file.size > MAX_IMG_SIZE) { toast.error('Image must be under 5 MB'); return false; }
    return true;
  };
...
function MultiImageField({ label, existingImages = [], onFilesSelect, onExistingCoverChange, maxCount = 100 }) {
  ...
  const validateFile = (f) => {
    ...
    if (f.size > MAX_IMG_SIZE) {
      toast.error(`${f.name} skipped — exceeds 5 MB`); return false;
    }
    return true;
  };
  ...
  return (
    <FormField label={label}>
      ...
      <div style={{ fontSize:'12px', color:'rgba(255,255,255,0.35)', marginBottom:'10px' }}>
        JPG / JPEG / PNG — max 5 MB each — up to {maxCount} photos.
