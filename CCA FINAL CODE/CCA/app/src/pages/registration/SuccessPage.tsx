...
// Simple barcode-style visual using canvas (code128 style bars)
function BarcodeDisplay({ value }: { value: string }) {
  ...
  useEffect(() => {
    ...
    for (const ch of chars) {
      ...
      bars.push((code % 3) + 1, (code % 2) + 1, ((code >> 2) % 3) + 1);
    }
    // Add quiet zone
    const quietZone = 10;
    const totalBarUnits = bars.reduce((a, b) => a + b, 0);
    const barWidth = (W - 2 * quietZone) / totalBarUnits;

    let x = quietZone;
    let isBlack = true;
    for (const units of bars) {
      ctx.fillStyle = isBlack ? "#0F172A" : "#ffffff";
      ctx.fillRect(x, 8, units * barWidth, H - 16);
      x += units * barWidth;
      isBlack = !isBlack;
    }
            ...
            <div className="space-y-7">
              ...
              <div className="rounded-[24px] border border-slate-200 bg-white p-6 shadow-md">
                <p className="text-xs uppercase tracking-widest text-slate-500 mb-3">
                  Registration ID / Barcode
                </p>
                <BarcodeDisplay value={registrationNumber} />
                <p className="mt-3 text-center font-mono text-sm font-bold text-[#0F172A]">
                  {registrationNumber}
                </p>
                <p className="mt-2 text-center text-xs text-slate-400">
                  Present this barcode at the academy check-in
                </p>
              </div>
