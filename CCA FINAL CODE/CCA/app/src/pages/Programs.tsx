...
function Programs() {
  ...
  const filtered = useMemo(() => {
    ...
    if (filters.season) list = list.filter((p) => p.category?.title === filters.season);

    // Location: compare against nested title
    if (filters.location) list = list.filter((p) => p.location?.title === filters.location);

    // Level: check membership in skillLevels array
    if (filters.level) list = list.filter((p) => p.skillLevels?.includes(filters.level));

    // Age group: check membership in ageGroups array
    if (filters.ageGroup) list = list.filter((p) => p.ageGroups?.includes(filters.ageGroup));
          ...
          {filtered.length === 0 ? (
            <div className="text-center py-20">
              <div className="max-w-md mx-auto bg-white rounded-[24px] p-8 shadow-lg border border-slate-200">
                ...
                <div className="mt-6">
                  <button onClick={() => setFilters({ season: "", location: "", level: "", ageGroup: "" })} className="inline-flex items-center justify-center rounded-full border border-slate-300 bg-white px-6 py-2 text-sm font-semibold text-[#0F172A] transition hover:border-[#F97316] hover:text-[#F97316]">
                    Clear Filters
                  </button>
                </div>
              </div>
