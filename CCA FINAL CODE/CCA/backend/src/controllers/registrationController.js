  ...
  try {
    ...
    const [data, total] = await Promise.all([
      getReg().find(filter)
        ...
        .populate('students',  'firstName lastName dob gender studentCode photoUrl')
        .populate({ path: 'batches', populate: { path: 'location', select: 'title city' } })
        .sort({ createdAt: -1 })
        .skip(skip)
        .limit(limit)
        .lean(),
...
// SUPER ADMIN ONLY. The one place a registration's batch assignment or a
// student's recorded details can be corrected after the fact. Every field
// that actually changes is captured in a human-readable diff and emailed
// to the parent automatically — they should never have to log in and
// guess what a staff member changed on their behalf.
//
// Body shape:
//   {
//     batches: ["<batchId>", ...],       // optional — full replacement list
//     students: [                          // optional — matched by index
//       { firstName, lastName, dob, gender }
//     ],
//     adminNote: "why this was changed"   // optional, stored, not emailed
//   }
exports.superAdminEdit = async (req, res) => {
  try {
    ...
    try {
      ...
      // The edit itself already succeeded and was saved — a failed email
      // should not roll that back or fail the whole request.
...
// Admin confirms physical check has been received.
// Sets paymentStatus → SUCCESS and status → CONFIRMED in one action.
